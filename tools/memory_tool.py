...
class MemoryTool(Tool):
    ...
    @tool_action("memory_add", "添加新记忆到系统中")
    def _add_memory(
        self,
        content: str = "",
    ...
    def _search_memory(
        ...
        for i, memory in enumerate(results, 1):
            type_label = memory_type_maps.get(memory.memory_type, memory.memory_type)
            content_preview = memory.content[:80] + "..." if len(memory.content) > 80 else memory.content
            formatted_results.append(
                f"{i}. [{type_label}] {content_preview}（重要性：{memory.importance:.2f}）"
            )
    ...
    def _get_summary(self, limit: int = 10) -> str:
        ...
        if stats["memories_by_type"]:
            ...
            for memory_type, type_stats in stats["memories_by_type"].items():
                ...
                type_label = memory_type_maps.get(memory_type, memory_type)
